...
char kayitlikullanicilar[50][50], isim[30], soyisim[30], girisismi[30], girissoyismi[30], telno[10], kayitlifaturalar[50][50], faturano[6], kayitlisigortalar[50][50],
saglik_sigorta[11], arac_sigorta[10], ev_mahalle_sigorta[20], ev_ilce_sigorta[20];
...
void dosyaya_yaz(int index)
{
    ...
    for (j = 0; j < index - 1; j++)
    {
        fprintf(bilgiler, "%s %d %.1f %.1f %.1f %.1f %d %d\n", 
                kayitlikullanicilar[j], kayitlisifreler[j],
                kayitliparalar[j], kayitliborclar[j],
                kayitlidolar[j], kayitlieuro[j],
                kayitliyatirim[j], kayitlihesapno[j]);
    }
    fclose(bilgiler);
}
...
void giris_yap()
{
    ...
    if ((bilgiler = fopen("bilgiler.txt", "r")) != NULL)
    {
        ...
        while (!feof(bilgiler))
        {
            fscanf(bilgiler, "%s %d %f %f %f %f %d %d", &kayitlikullanicilar[index], &kayitlisifreler[index], &kayitliparalar[index], &kayitliborclar[index], &kayitlidolar[index], 
            &kayitlieuro[index], &kayitliyatirim[index], &kayitlihesapno[index]);
            index++;
        }
        do
        {
